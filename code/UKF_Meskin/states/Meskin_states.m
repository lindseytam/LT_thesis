...
ukf.ProcessNoise = diag([0.2 0.1 .3 .4]); %stores the process noise covariance

T = 0.1; % [s] Filter sample time
timeVector = 0:T:5;

xTrue = readtable('Meskin_true.csv'); % true system values
xTrue = xTrue{:,:}; % converts csv data into matrix format

yMeas = readtable('Meskin_meas.csv'); % system measurements
yMeas = yMeas{:,:};
...
% uncomment this to generate new datasets each time
